...
birth_path_num = get_birthdate(birthdate)
	...
	def birthdate_message(birth_path_num)
	
	case birth_path_num 
	when number = 1
	message = "One is the leader. The number one indicates the ability to stand alond, and is a strong vibration. Ruled by the Sun."
	when 2
	...
	message ="Your number is not here!"
end

end

message = birthdate_message(birth_path_num)
